...
do
        ...
        samtools sort -@ ${SLURM_CPUS_PER_TASK} -o $workingdir/star/${i}.sorted.bam $workingdir/star/${i}_unsort.Aligned.out.bam
        samtools index $workingdir/star/${i}.sorted.bam

        ##  MARK DUPLICATES  ##
        java -jar $PICARD MarkDuplicates I=$workingdir/star/${i}.sorted.bam O=$workingdir/markdup/${i}.markdup.bam M=$workingdir/markdup/${i}.metrics.markdup.txt REMOVE_DUPLICATES=false VALIDATION_STRINGENCY=SILENT

        bamtools stats -in $workingdir/markdup/${i}.markdup.bam > $workingdir/markdup/${i}.markdup.dupstats.txt


        ## REMOVE DUPLICATES ##
        java -jar $PICARD MarkDuplicates I=$workingdir/star/${i}.sorted.bam O=$workingdir/markdup/${i}.rmdup.bam M=$workingdir/markdup/${i}.metrics.rmdup.txt REMOVE_DUPLICATES=true VALIDATION_STRINGENCY=SILENT
